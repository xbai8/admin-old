    ...
    public function step4()
    {
        ...
        switch ($step) {
            ...
            case 'site':
                ...
                // 写入管路员信息
            case 'admin':
                $smt = $db->prepare("insert into `{$database['prefix']}system_admin` (`create_at`, `update_at`, `role_id`, `pid`, `username`, `password`, `nickname`, `is_system`,`status`) values (:create_at, :update_at, :role_id, :pid, :username, :password, :nickname, :is_system, :status)");
                $data = [
                    'create_at'     => $dateTime,
                    'update_at'     => $dateTime,
                    'role_id'       => 1,
